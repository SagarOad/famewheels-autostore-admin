...
const {symbol} = useAppSelector((state)=>state.product)
  return (
    <>
      <h4>{singleProduct?.product_title}</h4>
      <div className="product-price fs-4 mb-3">
        {symbol}{singleProduct?.product_actual_price}
        <del className="f-w-700 fs-5 ms-1">{symbol}{singleProduct?.product_discounted_price}</del>
      </div>
      <div className="product-view product-modal-line">
        <h4 className="f-w-600">{ProductDetails}</h4>
        <p className="mb-0" dangerouslySetInnerHTML={{ __html: singleProduct?.product_description}}></p>
      </div>
