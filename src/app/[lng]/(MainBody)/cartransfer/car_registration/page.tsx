...
import { HtmlColumnData as HtmlColumnData, HtmlColumn, HtmlData, DealerColumn, CarImportColumn } from "@/Data/Form&Table/Table/DataTable/DataSourceData";
...
const HtmlSourcedData = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/carregistrationlist`, {
        params: {},
        headers: {
      ...
      setTotal(response?.data?.initial?.last_page);
  ...
  const {
    ...
    error,
    isLoading,
  } = useQuery(`carRegistration_${page}`, fetchData);
      ...
      <Card className="basic-data-table">
       {isLoading ? <Loading/> :  <CardBody>
          <div className="table-responsive">
            <DataTable className="theme-scrollbar" data={users} columns={CarImportColumn} striped highlightOnHover subHeader subHeaderComponent={subHeaderComponentMemo}/>
          </div>
