...
const HtmlSourcedData = () => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useQuery(`wallet_payment_${page}`, fetchData);
  ...
  const PaymentColumn: TableColumn<Payment>[] = [
  ...
  
    {
      name: "Action",
      cell: (row) =>{ return (
        <ul className="action simple-list d-flex flex-row" key={row?.order_id}>
          <li className="edit">
      ...
      <Card className="basic-data-table">
       {isLoading ? <Loading/> :  <CardBody>
          <div className="table-responsive">
            <DataTable className="theme-scrollbar" data={users} columns={PaymentColumn} striped highlightOnHover subHeader subHeaderComponent={subHeaderComponentMemo}/>
          </div>
