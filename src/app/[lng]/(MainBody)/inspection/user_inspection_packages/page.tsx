...
import { Inspection, InspectionPackages, Posts } from "@/Types/TableType";
...
const InspectionList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/usersinspectionpackagelist`,
        {
          headers: {
  ...
  const {
    ...
    error,
    isLoading,
  } = useQuery(`user_inspection_Packages_${page}${status}${update}`, fetchData);

  const filteredItems = inspectionData?.filter(
    (item: any) =>
      (item?.package_title &&
        item?.package_title.toLowerCase().includes(filterText.toLowerCase())) ||
      (item?.user_name &&
        item?.user_name.toLowerCase().includes(filterText.toLowerCase()))
  );

  const PostsColumn: TableColumn<InspectionPackages>[] = [
    {
      name: "User Name",
    ...
    },
    {
      ...
      cell: (row) => {
        return `PKR ${row.package_price}`;
      },
    },
    {
      name: "Package Quantity",
      selector: (row) => row.package_quantity,
    },
    {
      name: "Used Quantity",
      cell: (row) => {
        return row.package_quantity - row.remaining_quantity;
      },
    },
    {
