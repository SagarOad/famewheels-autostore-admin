...
const HtmlSourcedData = () => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useQuery(`pending_refund_${page}${getUpdate}`, fetchData);
  ...
  
    {
      ...
      cell: (row) => {
        return (
          ...
          <li className="delete">
            <button className="p-0 border-0 bg-transparent" onClick={()=>handleReject(row?.order_id)}>
